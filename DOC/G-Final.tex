...
\end{enumerate}
Calculating the flat index we have not used the strides as this is not neccessary. To explain the method let's look at an example with a four dimensional array where the $k$'th dimension has the size $d_k$. The flat index $i_flat$ for the array position \texttt{a[i1,i2,i3,i4]} is
...
$$i_{flat} = i_4 + d_4 (i_3 + d_3 (i_2 + (d_2 i_1))$$
