...
\end{enumerate}
Calculating the flat index we have not used the strides as this is not neccessary. To explain the method let's look at an example with a four dimensional array where the $k$'th dimension has the size $d_k$. The flat index $i_flat$ for the array position \texttt{a[i_1,i_2,i_3,i_4]} is
according to Section 3.2.3 calculated (with strides) like this:
$$i_flat = i_1 s_1 + i_2 s_2 + i_3 s_3 + i_4 s_4$$
Using the recursive definition of the strides we get
$$i_flat = 1*i_4 + d_4*i_3 + d_4 d_3 i_2 + d_4 d_3 d_2 i_1$$
...
\subsection{Parser.grm}
    \lstinputlisting[language=Yacc]{../SRC/Parser.grm}
\subsection{ParserTest.sml}
    \lstinputlisting[language=SML]{../SRC/ParserTest.sml}
\subsection{Compiler.sml}
    \lstinputlisting[language=SML]{../SRC/Compiler.sml}
\subsection{Type.sml}
    \lstinputlisting[language=SML]{../SRC/Type.sml}
\subsection{TpInterpret.sml}
    \lstinputlisting[language=SML]{../SRC/TpInterpret.sml}
\end{appendix}
\end{document}
